...
export const signup = async (req, res) => {
  try {
    ...
    if (password != confirm_password) {
      return res.status(400).json({ error: 'Passwords doen\'t match' });
    }
    ...
    // avatar placeholder https://avatar.iran.liara.run/public
    ...
    console.log('Error signup a user:', error.message);
...
export const logout = (req, res) => {
  try {
    ...
    console.log('Error logout a user:', error.message);
    res.status(500).json({ error: 'User logout error' });
  }
};
